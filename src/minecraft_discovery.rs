    ...
    loop {
        match socket.recv_from(&mut buffer) {
            Ok((size, _addr)) => {
                ...
                info!("📥 收到 LAN 广播: {}", message);

                // 解析消息: [MOTD]服务器名称[/MOTD][AD]端口[/AD]
                if let Some(server) = parse_lan_message(&message) {
                    info!("✓ 发现 Minecraft 服务器: {} (端口: {})", server.motd, server.port);
                    return Some(server);
                }
            }
            Err(e) => {
...
fn parse_lan_message(message: &str) -> Option<MinecraftServer> {
    // 提取 MOTD
...
mod tests {
    ...
    fn test_parse_lan_message() {
        ...
        let server = parse_lan_message(message).unwrap();
        assert_eq!(server.port, 25565);
        assert_eq!(server.motd, "My Test Server");
    }
